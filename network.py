...
class Network(nn.Module):
    def __init__(self, basename="resnet18", num_tool=6, num_verb=10, num_target=15, num_triplet=100, layer_size=8, num_heads=4, d_model=128, hr_output=False, use_ln=False):
        ...
        self.branch1 = Rendezvous(basename="resnet18", num_tool=6, num_verb=10, num_target=15, num_triplet=100, layer_size=8, num_heads=4, d_model=128, hr_output=False, use_ln=False)
        self.branch2 = Rendezvous(basename="resnet18", num_tool=6, num_verb=10, num_target=15, num_triplet=100, layer_size=8, num_heads=4, d_model=128, hr_output=False, use_ln=False)
...
class BaseModel(nn.Module):   
    def __init__(self, basename='resnet18', hr_output=False, *args):
        ...
        if basename == 'resnet18':
            self.basemodel      = basemodels.resnet18(weights=basemodels.ResNet18_Weights.IMAGENET1K_V1)     
        ...
        if basename == 'resnet50':
            self.basemodel      = basemodels.resnet50(weights=basemodels.ResNet50_Weights.IMAGENET1K_V1)
...
class WSL(nn.Module):
    def __init__(self, num_class, depth=64):
        ...
        nn.init.kaiming_normal_(self.cam.weight, mode='fan_in', nonlinearity='leaky_relu')
        self.elu   = nn.ELU()
...
class CAGAM(nn.Module):    
    def __init__(self, num_tool, num_verb, num_target, in_depth=512):
        ...
        nn.init.kaiming_normal_(self.verb_context.weight, mode='fan_in', nonlinearity='leaky_relu')     
        self.verb_query         = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)
        nn.init.kaiming_normal_(self.verb_query.weight, mode='fan_in', nonlinearity='leaky_relu')
        self.verb_tool_query    = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)
        nn.init.kaiming_normal_(self.verb_tool_query.weight, mode='fan_in', nonlinearity='leaky_relu')        
        self.verb_key           = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)
        nn.init.kaiming_normal_(self.verb_key.weight, mode='fan_in', nonlinearity='leaky_relu')
        self.verb_tool_key      = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)   
        nn.init.kaiming_normal_(self.verb_tool_key.weight, mode='fan_in', nonlinearity='leaky_relu')     
        self.verb_cmap          = nn.Conv2d(in_channels=out_depth, out_channels=num_verb, kernel_size=1)   
        nn.init.kaiming_normal_(self.verb_cmap.weight, mode='fan_in', nonlinearity='leaky_relu')    
        self.target_context     = nn.Conv2d(in_channels=in_depth, out_channels=out_depth, kernel_size=3, padding=1)
        nn.init.kaiming_normal_(self.target_context.weight, mode='fan_in', nonlinearity='leaky_relu')     
        self.target_query       = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)
        nn.init.kaiming_normal_(self.target_query.weight, mode='fan_in', nonlinearity='leaky_relu')
        self.target_tool_query  = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)  
        nn.init.kaiming_normal_(self.target_tool_query.weight, mode='fan_in', nonlinearity='leaky_relu')      
        self.target_key         = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1)
        nn.init.kaiming_normal_(self.target_key.weight, mode='fan_in', nonlinearity='leaky_relu')
        self.target_tool_key    = nn.Conv2d(in_channels=out_depth, out_channels=out_depth, kernel_size=1) 
        nn.init.kaiming_normal_(self.target_tool_key.weight, mode='fan_in', nonlinearity='leaky_relu')       
        self.target_cmap        = nn.Conv2d(in_channels=out_depth, out_channels=num_target, kernel_size=1)
        nn.init.kaiming_normal_(self.target_cmap.weight, mode='fan_in', nonlinearity='leaky_relu')        
        self.gmp       = nn.AdaptiveMaxPool2d((1,1))
        self.elu       = nn.ELU()    
