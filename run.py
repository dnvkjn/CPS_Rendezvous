...
parser.add_argument('--version', type=int, default=3,  help='Model version control (for keeping several versions)') 
...
# data
parser.add_argument('--data_dir', type=str, default='/home/jhuangcj/tom/CholecT45/CholecT45', help='path to dataset?')
...
model_dir       = "/home/jhuangcj/tom/rend/__checkpoint__/run_{}".format(version)
...
# Group 107
# for Rendezvous model trained by semi-supervised learning(cross pseudo supervision)
def semi_train(sup_dataloader, unsup_dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers_l, optimizers_r, scheduler_l, scheduler_r, epoch):
    start = time.time()
    for (batch, ((img_sup, (y1_sup, y2_sup, y3_sup, y4_sup)), (img_unsup, (y1_unsup, y2_unsup, y3_unsup, y4_unsup)))) in enumerate(zip(sup_dataloader, unsup_dataloader)):
        ...
        model.train()
        # The model has two sub-networks and each of them is a Rendezvous model 

        # outputs of 1st subnetwork
        # l: 1st subnetwork
        # r: 2nd subnetwork
        tool_sup_l, verb_sup_l, target_sup_l, triplet_sup_l = model(img_sup, step=1)
        tool_unsup_l, verb_unsup_l, target_unsup_l, triplet_unsup_l = model(img_unsup, step=1)

        # outputs of 2nd subnetwork
        tool_sup_r, verb_sup_r, target_sup_r, triplet_sup_r = model(img_sup, step=2)
        tool_unsup_r, verb_unsup_r, target_unsup_r, triplet_unsup_r = model(img_unsup, step=2)

        # cam: class activation map
        # logit: logit that output by the model
        cam_i_sup_l, logit_i_sup_l  = tool_sup_l
        cam_v_sup_l, logit_v_sup_l  = verb_sup_l
        cam_t_sup_l, logit_t_sup_l  = target_sup_l
        logit_ivt_sup_l             = triplet_sup_l

        cam_i_unsup_l, logit_i_unsup_l  = tool_unsup_l
        cam_v_unsup_l, logit_v_unsup_l  = verb_unsup_l
        cam_t_unsup_l, logit_t_unsup_l  = target_unsup_l
        logit_ivt_unsup_l               = triplet_unsup_l

        cam_i_sup_r, logit_i_sup_r  = tool_sup_r
        cam_v_sup_r, logit_v_sup_r  = verb_sup_r
        cam_t_sup_r, logit_t_sup_r  = target_sup_r
        logit_ivt_sup_r             = triplet_sup_r

        cam_i_unsup_r, logit_i_unsup_r  = tool_unsup_r
        cam_v_unsup_r, logit_v_unsup_r  = verb_unsup_r
        cam_t_unsup_r, logit_t_unsup_r  = target_unsup_r
        logit_ivt_unsup_r               = triplet_unsup_r

        pred_tool_l = torch.cat((logit_i_sup_l, logit_i_unsup_l), dim=0)
        pred_verb_l = torch.cat((logit_v_sup_l, logit_v_unsup_l), dim=0)
        pred_target_l = torch.cat((logit_t_sup_l, logit_t_unsup_l), dim=0)
        pred_triplet_l = torch.cat((logit_ivt_sup_l, logit_ivt_unsup_l), dim=0)

        pred_tool_r = torch.cat((logit_i_sup_r, logit_i_unsup_r), dim=0)
        pred_verb_r = torch.cat((logit_v_sup_r, logit_v_unsup_r), dim=0)
        pred_target_r = torch.cat((logit_t_sup_r, logit_t_unsup_r), dim=0)
        pred_triplet_r = torch.cat((logit_ivt_sup_r, logit_ivt_unsup_r), dim=0)

        label_tool = torch.cat((y1_sup, y1_unsup), dim=0)
        label_verb = torch.cat((y2_sup, y2_unsup), dim=0)
        label_target = torch.cat((y3_sup, y3_unsup), dim=0)
        label_triplet = torch.cat((y4_sup, y4_unsup), dim=0)

        # generate predictions by the outputs of model
        # function.convert() converts the prediction logits to prediction labels
        cps_label_tool_l = function.convert(pred_tool_l, label_tool)
        cps_label_verb_l = function.convert(pred_verb_l, label_verb)
        cps_label_target_l = function.convert(pred_target_l, label_target)
        cps_label_triplet_l = function.convert(pred_triplet_l, label_triplet)

        cps_label_tool_r = function.convert(pred_tool_r, label_tool)
        cps_label_verb_r = function.convert(pred_verb_r, label_verb)
        cps_label_target_r = function.convert(pred_target_r, label_target)
        cps_label_triplet_r = function.convert(pred_triplet_r, label_triplet)

        # cps: cross pseudo supervision
        # pred: logits that output by model
        # label: prediction labels that generated by logits 
        cps_loss_i          = loss_fn_i(pred_tool_l, cps_label_tool_r) + loss_fn_i(pred_tool_r, cps_label_tool_l)
        cps_loss_v          = loss_fn_v(pred_verb_l, cps_label_verb_r) + loss_fn_v(pred_verb_r, cps_label_verb_l)
        cps_loss_t          = loss_fn_t(pred_target_l, cps_label_target_r) + loss_fn_t(pred_target_r, cps_label_target_l)
        cps_loss_ivt        = loss_fn_ivt(pred_triplet_l, cps_label_triplet_r) + loss_fn_ivt(pred_triplet_r, cps_label_triplet_l) 
        cps_loss            = (cps_loss_i) + (cps_loss_v) + (cps_loss_t) + cps_loss_ivt

        # labelled data will be supervised by their ground truths
        sup_loss_i_l = loss_fn_i(logit_i_sup_l, y1_sup.float())
        sup_loss_v_l = loss_fn_v(logit_v_sup_l, y2_sup.float())
        sup_loss_t_l = loss_fn_t(logit_t_sup_l, y3_sup.float())
        sup_loss_ivt_l = loss_fn_ivt(logit_ivt_sup_l, y4_sup.float())
        sup_loss_l = sup_loss_i_l + sup_loss_v_l + sup_loss_t_l + sup_loss_ivt_l

        sup_loss_i_r = loss_fn_i(logit_i_sup_r, y1_sup.float())
        sup_loss_v_r = loss_fn_v(logit_v_sup_r, y2_sup.float())
        sup_loss_t_r = loss_fn_t(logit_t_sup_r, y3_sup.float())
        sup_loss_ivt_r = loss_fn_ivt(logit_ivt_sup_r, y4_sup.float())
        sup_loss_r = sup_loss_i_r + sup_loss_v_r + sup_loss_t_r + sup_loss_ivt_r

        loss = cps_loss + sup_loss_l + sup_loss_r
        
        # brach 1: 1st subnetwork
        # branch 2: 2nd subnetwork
        for param in model.branch1.parameters():
            param.grad = None
        for param in model.branch2.parameters():
            param.grad = None
        loss.backward()
        for opt in optimizers_l:
            opt.step()
        for opt in optimizers_r:
            opt.step()

    for sch in scheduler_l:
        sch.step()
    for sch in scheduler_r:
        sch.step()

    print(f'completed | cps_loss => i: [{cps_loss_i.item():.4f}] v: [{cps_loss_v.item():.4f}] t: [{cps_loss_t.item():.4f}] ivt: [{cps_loss_ivt.item():.4f}] >> eta: {(time.time() - start):.2f} secs', file=open(logfile, 'a+'))
    print(f'completed | sup_loss_l => i: [{sup_loss_i_l.item():.4f}] v: [{sup_loss_v_l.item():.4f}] t: [{sup_loss_t_l.item():.4f}] ivt: [{sup_loss_ivt_l.item():.4f}] >> eta: {(time.time() - start):.2f} secs', file=open(logfile, 'a+'))
    print(f'completed | sup_loss_r => i: [{sup_loss_i_r.item():.4f}] v: [{sup_loss_v_r.item():.4f}] t: [{sup_loss_t_r.item():.4f}] ivt: [{sup_loss_ivt_r.item():.4f}] >> eta: {(time.time() - start):.2f} secs', file=open(logfile, 'a+'))
...
# Group 107
# for testing of Rendezvous model that trained by semi-supervised learning(cross pseudo supervision)
def semi_test(dataloader, model, activation, final_eval=False):
    size = len(dataloader.dataset)
    num_batches = len(dataloader)   
    mAP_l.reset()
    mAP_r.reset()
    ...
    with torch.no_grad():
        for batch, (img, (y1, y2, y3, y4)) in enumerate(dataloader):
            img, y1, y2, y3, y4 = img.cuda(), y1.cuda(), y2.cuda(), y3.cuda(), y4.cuda()            
            model.eval()
            tool_l, verb_l, target_l, triplet_l = model(img, step=1)
            tool_r, verb_r, target_r, triplet_r = model(img, step=2)
            if final_eval and not set_chlg_eval:
                cam_i_l, logit_i_l = tool_l
                cam_v_l, logit_v_l = verb_l
                cam_t_l, logit_t_l = target_l
                cam_i_r, logit_i_r = tool_r
                cam_v_r, logit_v_r = verb_r
                cam_t_r, logit_t_r = target_r
                mAPi_l.update(y1.float().detach().cpu(), activation(logit_i_l).detach().cpu()) 
                mAPv_l.update(y2.float().detach().cpu(), activation(logit_v_l).detach().cpu()) 
                mAPt_l.update(y3.float().detach().cpu(), activation(logit_t_l).detach().cpu()) 
                mAPi_r.update(y1.float().detach().cpu(), activation(logit_i_r).detach().cpu()) 
                mAPv_r.update(y2.float().detach().cpu(), activation(logit_v_r).detach().cpu()) 
                mAPt_r.update(y3.float().detach().cpu(), activation(logit_t_r).detach().cpu())
            mAP_l.update(y4.float().detach().cpu(), activation(triplet_l).detach().cpu()) 
            mAP_r.update(y4.float().detach().cpu(), activation(triplet_r).detach().cpu())
...
def semi_weight_mgt(score_l, score_r, epoch):
    # hyperparameter selection based on validation set
    global benchmark
    if score_l > benchmark.item() and score_r > benchmark.item():
        torch.save(model.state_dict(), ckpt_path)
        benchmark = max(score_l, score_r)
        print(f'>>> Saving checkpoint for epoch {epoch+1} at {ckpt_path}, time {time.ctime()} ', file=open(logfile, 'a+'))  
        return "increased"
    else:
        return "decreased"
...
# Ratio of labelled and unlabelled data
supervised_ratio = 0.5 
unsupervised_ratio = 0.5

supervised_train_dataset, unsupervised_train_dataset = data.random_split(train_dataset, [supervised_ratio, unsupervised_ratio])
supervised_train_dataloader = DataLoader(supervised_train_dataset, batch_size=batch_size, shuffle=True, prefetch_factor=3*batch_size, num_workers=3, pin_memory=True, persistent_workers=True, drop_last=True)
unsupervised_train_dataloader = DataLoader(unsupervised_train_dataset, batch_size=batch_size, shuffle=True, prefetch_factor=3*batch_size, num_workers=3, pin_memory=True, persistent_workers=True, drop_last=True)
# Group 107
...
model = network.Network('resnet18', hr_output=hr_output, use_ln=use_ln).cuda()
...
# Group 107
mAP_l = ivtmetrics.Recognition(100)
mAP_l.reset_global()
if not set_chlg_eval:
    mAPi_l = ivtmetrics.Recognition(6)
    mAPv_l = ivtmetrics.Recognition(10)
    mAPt_l = ivtmetrics.Recognition(15)
    mAPi_l.reset_global()
    mAPv_l.reset_global()
    mAPt_l.reset_global()

mAP_r = ivtmetrics.Recognition(100)
mAP_r.reset_global()
if not set_chlg_eval:
    mAPi_r = ivtmetrics.Recognition(6)
    mAPv_r = ivtmetrics.Recognition(10)
    mAPt_r = ivtmetrics.Recognition(15)
    mAPi_r.reset_global()
    mAPv_r.reset_global()
    mAPt_r.reset_global()
...
# Group 107
module_i_l        = list(set(model.branch1.parameters()) - set(model.branch1.encoder.cagam.parameters()) - set(model.branch1.encoder.bottleneck.parameters()) - set(model.branch1.decoder.parameters()))
module_ivt_l      = list(set(model.branch1.encoder.bottleneck.parameters()).union(set(model.branch1.decoder.parameters())))
module_vt_l       = model.branch1.encoder.cagam.parameters()

module_i_r        = list(set(model.branch2.parameters()) - set(model.branch2.encoder.cagam.parameters()) - set(model.branch2.encoder.bottleneck.parameters()) - set(model.branch2.decoder.parameters()))
module_ivt_r      = list(set(model.branch2.encoder.bottleneck.parameters()).union(set(model.branch2.decoder.parameters())))
module_vt_r       = model.branch2.encoder.cagam.parameters()

optimizer_i_l     = torch.optim.SGD(module_i_l, lr=wp_lr[0], weight_decay=weight_decay)
scheduler_ia_l    = torch.optim.lr_scheduler.LinearLR(optimizer_i_l, start_factor=power, total_iters=warmups[0])
scheduler_ib_l    = torch.optim.lr_scheduler.ExponentialLR(optimizer_i_l, gamma=decay_rate)
scheduler_i_l     = torch.optim.lr_scheduler.SequentialLR(optimizer_i_l, schedulers=[scheduler_ia_l, scheduler_ib_l], milestones=[warmups[0]+1])

optimizer_i_r     = torch.optim.SGD(module_i_r, lr=wp_lr[0], weight_decay=weight_decay)
scheduler_ia_r    = torch.optim.lr_scheduler.LinearLR(optimizer_i_r, start_factor=power, total_iters=warmups[0])
scheduler_ib_r    = torch.optim.lr_scheduler.ExponentialLR(optimizer_i_r, gamma=decay_rate)
scheduler_i_r     = torch.optim.lr_scheduler.SequentialLR(optimizer_i_r, schedulers=[scheduler_ia_r, scheduler_ib_r], milestones=[warmups[0]+1])

optimizer_vt_l    = torch.optim.SGD(module_vt_l, lr=wp_lr[1], weight_decay=weight_decay)
scheduler_vta_l   = torch.optim.lr_scheduler.LinearLR(optimizer_vt_l, start_factor=power, total_iters=warmups[1])
scheduler_vtb_l   = torch.optim.lr_scheduler.ExponentialLR(optimizer_vt_l, gamma=decay_rate)
scheduler_vt_l    = torch.optim.lr_scheduler.SequentialLR(optimizer_vt_l, schedulers=[scheduler_vta_l, scheduler_vtb_l], milestones=[warmups[1]+1])

optimizer_vt_r    = torch.optim.SGD(module_vt_r, lr=wp_lr[1], weight_decay=weight_decay)
scheduler_vta_r   = torch.optim.lr_scheduler.LinearLR(optimizer_vt_r, start_factor=power, total_iters=warmups[1])
scheduler_vtb_r   = torch.optim.lr_scheduler.ExponentialLR(optimizer_vt_r, gamma=decay_rate)
scheduler_vt_r    = torch.optim.lr_scheduler.SequentialLR(optimizer_vt_r, schedulers=[scheduler_vta_r, scheduler_vtb_r], milestones=[warmups[1]+1])

optimizer_ivt_l   = torch.optim.SGD(module_ivt_l, lr=wp_lr[2], weight_decay=weight_decay)
scheduler_ivta_l  = torch.optim.lr_scheduler.LinearLR(optimizer_ivt_l, start_factor=power, total_iters=warmups[2])
scheduler_ivtb_l  = torch.optim.lr_scheduler.ExponentialLR(optimizer_ivt_l, gamma=decay_rate)
scheduler_ivt_l   = torch.optim.lr_scheduler.SequentialLR(optimizer_ivt_l, schedulers=[scheduler_ivta_l, scheduler_ivtb_l], milestones=[warmups[2]+1])

optimizer_ivt_r   = torch.optim.SGD(module_ivt_r, lr=wp_lr[2], weight_decay=weight_decay)
scheduler_ivta_r  = torch.optim.lr_scheduler.LinearLR(optimizer_ivt_r, start_factor=power, total_iters=warmups[2])
scheduler_ivtb_r  = torch.optim.lr_scheduler.ExponentialLR(optimizer_ivt_r, gamma=decay_rate)
scheduler_ivt_r   = torch.optim.lr_scheduler.SequentialLR(optimizer_ivt_r, schedulers=[scheduler_ivta_r, scheduler_ivtb_r], milestones=[warmups[2]+1])

lr_schedulers_l   = [scheduler_i_l, scheduler_vt_l, scheduler_ivt_l]
lr_schedulers_r   = [scheduler_i_r, scheduler_vt_r, scheduler_ivt_r]
optimizers_l      = [optimizer_i_l, optimizer_vt_l, optimizer_ivt_l]
optimizers_r      = [optimizer_i_r, optimizer_vt_r, optimizer_ivt_r]
# Group 107
...
header3 = "** LR Config: Init_l: {} | Init_r: {} | Peak: {} | Warmup Epoch: {} | Rise: {} | Decay {} | train params {} | all params {} **".format([float(f'{sch.get_last_lr()[0]:.6f}') for sch in lr_schedulers_l], [float(f'{sch.get_last_lr()[0]:.6f}') for sch in lr_schedulers_r], [float(f'{v:.6f}') for v in wp_lr], warmups, power, decay_rate, pytorch_train_params, pytorch_total_params)
...
if is_train:
    if __name__ == '__main__':
        for epoch in range(0,epochs):
            try:
                # Train
                print("Traning | lr_l: {} | lr_r: {} | epoch {}".format([lr.get_last_lr() for lr in lr_schedulers_l], [lr.get_last_lr() for lr in lr_schedulers_r], epoch), end=" | ", file=open(logfile, 'a+'))  
                semi_train(supervised_train_dataloader, unsupervised_train_dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers_l, optimizers_r, lr_schedulers_l, lr_schedulers_r, epoch)
                ...
                if epoch % val_interval == 0:
                    ...
                    semi_test(val_dataloader, model, activation, final_eval=False)
                    #behaviour = weight_mgt(mAP.compute_video_AP()['mAP'], epoch=epoch)
                    behaviour = semi_weight_mgt(mAP_l.compute_video_AP()['mAP'], mAP_r.compute_video_AP()['mAP'], epoch=epoch)
                    print("\t\t\t\t video-wise | eta {:.2f} secs | mAP_l => ivt: [{:.5f}] | mAP_r => ivt: [{:.5f}] ".format( (time.time() - start), mAP_l.compute_video_AP('ivt', ignore_null=set_chlg_eval)['mAP'], mAP_r.compute_video_AP('ivt', ignore_null=set_chlg_eval)['mAP']), file=open(logfile, 'a+'))      
...
if is_test:
    ...
    for test_dataloader in test_dataloaders:
        semi_test(test_dataloader, model, activation, final_eval=True)
    if set_chlg_eval:
        mAP_i_l = mAP_l.compute_video_AP('i', ignore_null=set_chlg_eval)
        mAP_v_l = mAP_l.compute_video_AP('v', ignore_null=set_chlg_eval)
        mAP_t_l = mAP_l.compute_video_AP('t', ignore_null=set_chlg_eval)
        mAP_i_r = mAP_r.compute_video_AP('i', ignore_null=set_chlg_eval)
        mAP_v_r = mAP_r.compute_video_AP('v', ignore_null=set_chlg_eval)
        mAP_t_r = mAP_r.compute_video_AP('t', ignore_null=set_chlg_eval)
    else:
        mAP_i_l = mAPi_l.compute_video_AP(ignore_null=set_chlg_eval)
        mAP_v_l = mAPv_l.compute_video_AP(ignore_null=set_chlg_eval)
        mAP_t_l = mAPt_l.compute_video_AP(ignore_null=set_chlg_eval)
        mAP_i_r = mAPi_r.compute_video_AP(ignore_null=set_chlg_eval)
        mAP_v_r = mAPv_r.compute_video_AP(ignore_null=set_chlg_eval)
        mAP_t_r = mAPt_r.compute_video_AP(ignore_null=set_chlg_eval)
    mAP_iv_l = mAP_l.compute_video_AP('iv', ignore_null=set_chlg_eval)
    mAP_it_l = mAP_l.compute_video_AP('it', ignore_null=set_chlg_eval)
    mAP_ivt_l = mAP_l.compute_video_AP('ivt', ignore_null=set_chlg_eval) 
    mAP_iv_r = mAP_r.compute_video_AP('iv', ignore_null=set_chlg_eval)
    mAP_it_r = mAP_r.compute_video_AP('it', ignore_null=set_chlg_eval)
    mAP_ivt_r = mAP_r.compute_video_AP('ivt', ignore_null=set_chlg_eval) 
    
    # results of 1st subnetwork
    print('-'*50, file=open(logfile, 'a+'))
    print('Test Results of 1st subnetwork\nPer-category AP: ', file=open(logfile, 'a+'))
    print(f'I   : {mAP_i_l["AP"]}', file=open(logfile, 'a+'))
    print(f'V   : {mAP_v_l["AP"]}', file=open(logfile, 'a+'))
    print(f'T   : {mAP_t_l["AP"]}', file=open(logfile, 'a+'))
    print(f'IV  : {mAP_iv_l["AP"]}', file=open(logfile, 'a+'))
    print(f'IT  : {mAP_it_l["AP"]}', file=open(logfile, 'a+'))
    print(f'IVT : {mAP_ivt_l["AP"]}', file=open(logfile, 'a+'))
    print('-'*50, file=open(logfile, 'a+'))
    print(f'Mean AP:  I  |  V  |  T  |  IV  |  IT  |  IVT ', file=open(logfile, 'a+'))
    print(f':::::: : {mAP_i_l["mAP"]:.4f} | {mAP_v_l["mAP"]:.4f} | {mAP_t_l["mAP"]:.4f} | {mAP_iv_l["mAP"]:.4f} | {mAP_it_l["mAP"]:.4f} | {mAP_ivt_l["mAP"]:.4f} ', file=open(logfile, 'a+'))
    print('='*50, file=open(logfile, 'a+'))

    # results of 2nd subnetwork
    print('-'*50, file=open(logfile, 'a+'))
    print('Test Results of 2nd subnetwork\nPer-category AP: ', file=open(logfile, 'a+'))
    print(f'I   : {mAP_i_r["AP"]}', file=open(logfile, 'a+'))
    print(f'V   : {mAP_v_r["AP"]}', file=open(logfile, 'a+'))
    print(f'T   : {mAP_t_r["AP"]}', file=open(logfile, 'a+'))
    print(f'IV  : {mAP_iv_r["AP"]}', file=open(logfile, 'a+'))
    print(f'IT  : {mAP_it_r["AP"]}', file=open(logfile, 'a+'))
    print(f'IVT : {mAP_ivt_r["AP"]}', file=open(logfile, 'a+'))
    print('-'*50, file=open(logfile, 'a+'))
    print(f'Mean AP:  I  |  V  |  T  |  IV  |  IT  |  IVT ', file=open(logfile, 'a+'))
    print(f':::::: : {mAP_i_r["mAP"]:.4f} | {mAP_v_r["mAP"]:.4f} | {mAP_t_r["mAP"]:.4f} | {mAP_iv_r["mAP"]:.4f} | {mAP_it_r["mAP"]:.4f} | {mAP_ivt_r["mAP"]:.4f} ', file=open(logfile, 'a+'))
    print('='*50, file=open(logfile, 'a+'))
